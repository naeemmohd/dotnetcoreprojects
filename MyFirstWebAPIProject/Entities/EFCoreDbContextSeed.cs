...
namespace MyFirstWebAPIProject.Entities
{
    public class EFCoreDbContextSeed
    {
        public static void Seed(EFCoreDbContext context)
        {
            ...
            var removeAllStudents = context.Students.Where(s=>s.StudentId >0).ToArray();
            context.Students.RemoveRange(removeAllStudents);
            context.SaveChanges();
            ...
            var removeAllStandards = context.Standards.Where(s=>s.StandardId >0).ToArray();
            context.Standards.RemoveRange(removeAllStandards);
            context.SaveChanges();
